...
const getHeader = options => {
  let headerData = options.header.version << 5

  if (options.header.downstreamFlag) {
    headerData = headerData ^ 0b10000
  }
  if (options.header.compressedFlag) {
    headerData = headerData ^ 0b1000
  }
  headerData |= Math.min(options.header.type, 7)

  // TODO: Remove after veryfing that the Bits are right
  console.log(headerData.toString(2))
  console.log(
    `version (3) = ${options.header.version}, downstream = ${options.header.downstreamFlag}, compressed = ${options.header.compressedFlag}, type (3) = ${options.header.type}`
  )
  return headerData
}
