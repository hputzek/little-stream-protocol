...
const GENERAL_HEADER_BITS = 8
const MESSAGE_HEADER_BITS = 32
const FIRST_FRAME_ADDITIONAL_HEADER_BITS = 24
const FIRST_FRAME_ADDITIONAL_HEADER_BYTES =
  FIRST_FRAME_ADDITIONAL_HEADER_BITS / 8

const FIRST_FRAME_HEADER_BYTES =
  (GENERAL_HEADER_BITS +
    MESSAGE_HEADER_BITS +
    FIRST_FRAME_ADDITIONAL_HEADER_BITS) /
  8

const ONGOING_FRAMES_HEADER_BYTES =
  (GENERAL_HEADER_BITS + MESSAGE_HEADER_BITS) / 8
...
export const getFrame = frameOptions => {
  ...
  const fragmentedMessages = chunkArrayBuffer(
    payload,
    options.maxPacketSize - FIRST_FRAME_HEADER_BYTES
  )

  const payloadPlusHeaderSizes =
    fragmentedMessages.length * ONGOING_FRAMES_HEADER_BYTES +
    FIRST_FRAME_ADDITIONAL_HEADER_BYTES +
    payload.byteLength
  console.log(` ------- Frame number ${frameCount} ------`)
  console.log(`Byte length pixel processed payload: ${payload.byteLength}`)
  console.log(
    `Byte length pixel processed payload + headers: ${payloadPlusHeaderSizes}`
  )

  const output = fragmentedMessages.map((message, index) => {
    const isFirstFragment = () => index === 0
    const HEADER_SIZE = isFirstFragment()
      ? FIRST_FRAME_HEADER_BYTES
      : ONGOING_FRAMES_HEADER_BYTES
    ...
    if (isFirstFragment()) {
      headerView.setUint16(5, payloadPlusHeaderSizes, true)
