...
const setHeaderData = (settings, headerDataView) => {
  ...
  headerDataView.setUint16(
    3,
    settings.payload.length / (settings.type === 'rgb' ? 3 : 4),
    true
  )
}
...
export const getFrame = options => {
  ...
  settings.payload =
    typeof settings.payload === Array
      ? settings.payload.flat()
      : settings.payload
  const HEADER_SIZE = 5 // How many bytes does the header need?
  const MESSAGE_SIZE = settings.payload.length
  const BUFFER_SIZE = settings.payload.length + HEADER_SIZE + 1 // calculate how big the buffer must be
  const buffer = new ArrayBuffer(BUFFER_SIZE)

  const header = new DataView(buffer, 0, HEADER_SIZE)
  const message = new DataView(buffer, HEADER_SIZE, MESSAGE_SIZE)
  setHeaderData(settings, header)
  // put pixel values into message
  settings.payload.map((value, index) => {
